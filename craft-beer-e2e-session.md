# Craft Beer Scheduling – E2E Session Report
...
E2E test of the stateful interviewer agent (run-interview2!) with craft beer surrogate expert.

Completed 10 ASCRs across process, resources, data, and optimality conversations. Created 1 pedagogical MiniZinc DPO (single-batch residence time, 11-day makespan).

Bugs found and fixed:
1. optimality/warm-up completeness-test checked wrong keys (copy-paste from process/warm-up), causing the agent to loop until budget exhausted. Fixed in warm_up.clj.
2. Calling parametric DS (resources/challenges) without question_topic causes ASCR schema assertion failure. Need better error message for the orchestrator LLM.
3. delegate_DS_interview intermittently returns empty output despite successful completion (process/flow-shop, both data/orm-modeling interviews).
4. Two data/orm-modeling ASCRs left in-progress: sales-forecast-and-customer-orders (budget exhausted), tank-fermenter-brite-board (budget remaining but not marked complete).
...
| --- | --- | --- | --- | --- |
| 1) Receive & store ingredients + packaging | Malt (25 kg sacks), specialty grains, hops (5–20 kg boxes, cold), yeast (lab pitch bags),
      brewing salts/chemicals, CO2; cans (355 mL + lids), case trays, labels | Inventory put away and available; hops in cold storage; grains staged | Forklift/pallet jack, dry storage, hop cooler (2–4°C), small chemical cabinet | 1–3 hours per delivery (varies with pallet count) |
